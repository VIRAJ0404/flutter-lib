...
// Full widget card builder (unchanged behaviors), minor compatibility fixes:
// - Uses withOpacity instead of withValues for alpha.
// - Provides non-null fallback colors where needed.
...
double? _resolveValue(DashWidget dw) {
  ...
  if (dw.readTopic != null && dw.readTopic!.isNotEmpty) {
    final v = TopicStore.I.latest(dw.readTopic!)?.value;
      ...
      content = Center(
        child: OutlinedButton.icon(
          style: OutlinedButton.styleFrom(
            side: BorderSide(
                color: dw.color ?? Theme.of(context).colorScheme.primary,
                width: 1.5),
            foregroundColor: dw.color ?? Theme.of(context).colorScheme.primary,
          ),
